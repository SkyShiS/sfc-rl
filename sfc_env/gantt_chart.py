...
def plot_gantt(env):
    ...
    for s in completed_sfcs:
        for n in s.nodes:
            if n.start_time is not np.nan and n.vm_node is not None:
                ...
                #     s.idx, n.vnf_idx, n.idx, n.v_type, n.start_time,
                ...
                plt.text(n.start_time, n.vm_node.idx,
                         ' J%s %sth\nid%s\nf%s\nt%s' % (s.idx, n.vnf_idx, n.idx, n.v_type, round(n.start_time, 2)),
                         color="black",
                         size=6)
    ...
    #         plt.barh(i, t[j], left=(np.sum(t[:j])))
    #         plt.text(np.sum(t[:j]) + t[j] / 8, i, 'J%s\nT%s' % ((job[j]), t[j]), color="white", size=8)
    plt.yticks(np.arange(configs['vm']['num']), np.arange(configs['vm']['num']))
    plt.show()
